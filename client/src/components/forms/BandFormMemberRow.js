...
const BandFormMemberRow = ({ member, band, status, edit }) => {
  ...
  const instruments = useSelector((state) =>
    member.profileInfo.instruments.map(
  ...
  const handleRemoveMemberClick = async (e) => {
    e.preventDefault();
    await dispatch(
      fetchAndDeleteMember(band.id, member.id, status === 'confirmed')
    );
  };
  ...
  return (
    <div className={bandStyles.bandFormMemberRow}>
      <div className={bandStyles.bandFormMemberRowPicAndInfo}>
        ...
        />
        <div>
          <h5 className={bandStyles.bandFormMemberRowName}>
            {member.firstName + ' ' + member.lastName}
          </h5>
          <span className={bandStyles.bandFormMemberRowInfo}>
            {isOwner
              ? `Owner: ${instruments
      ...
      {!isOwner && edit ? (
        <button
          className={bandStyles.removeMemberButton}
