...
const EditBandForm = () => {
    ...
    return (
        <form className={bandStyles.editBandForm} onSubmit={handleSubmit} method="" action="">
            <h2 className={bandStyles.modalFormTitle}>{edit ? "Edit Band" : band.name}</h2>
            ...
            <p>
                <label className="labels" htmlFor="bandName">Band Name
                {
            ...
            <div className="form-control-group">
            <p>
                <label className="labels" htmlFor="bandStyle">Band Style
                {
                   edit ? <span className="redText">*</span> : <></>
                }
                </label>
            </p>
            <select className={"form-control" + (edit ? "" : " " + bandStyles.disabled)} name="bandStyle" value={styleId} onChange={(e) => setStyleId(`${e.target.value}`)}>
            ...
            <div className={bandStyles.bandFormMemberArea}>
                { members.map((member) => <BandFormMemberRow edit={edit} status="confirmed" band={band} member={member}/>) }
                { pendingMembers.map((member) => <BandFormMemberRow edit={edit} status="pending" band={band} member={member} />) }
            </div>
            </div>
            {
                edit ?
