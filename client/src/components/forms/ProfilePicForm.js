...
import Cropper from '../universal/Cropper';
...
const ProfilePicForm = () => {
  ...
  const {
    get: { crop, fileName, pic, picRef, user },
    set: { setCrop, setFileName, setPic },
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await dispatch(
      putAndUpdateProfilePic(user.id, await getCroppedImg(picRef.current, crop))
    );
  ...
  return (
    <>
      <div>
        <label className={styles.uploadLabel} onClick={handleLabelClick}>
      ...
      {pic ? (
        <div>
          <form className={styles.picForm} onSubmit={handleSubmit}>
            <Cropper src={pic} />
