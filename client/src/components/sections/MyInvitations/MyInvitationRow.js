      ...
      <div className={invStyles.userInfo}>
        ...
        <div className={invStyles.invitationRowTextContainer}>
          <div className={invStyles.invitationRowNameAndStatusContainer}>
            <h3 className={invStyles.invitationRowUser}>
              {otherUser.firstName + ' ' + otherUser.lastName}
            </h3>
            <span
              className={
                invStyles.invitationRowStatus +
                (invitation.status === 'Pending'
                  ? ' ' + invStyles.pendingStatus
                  : invitation.status === 'Accepted'
                  ? ' ' + invStyles.acceptedStatus
                  : ' ' + invStyles.declinedStatus)
              }
            >
              {invitation.status}
            </span>
          ...
          {invitation.message ? (
            <p className={invStyles.invitationRowMessage}>
              ...
              <span className={invStyles.invitationRowDate}>
                {'  ' + invitationRowDate(invitation.createdAt)}
              </span>
            </p>
          ) : (
